...
"""%( txt1, txt2, txt3, txt4, os.path.expanduser('~/linuxcnc'), txt5,
    ...
    def init_probe(self):
        ...
        if probe == 'versaprobe':
            ...
            # SSE used as a 'no probe' object for comparison later
            self._probeLibrary = SSE
...
#                    QtCore.Qt.Key_F3,QtCore.Qt.Key_F4,QtCore.Qt.Key_F5):
    ...
    def btn_start_clicked(self, obj):
        ...
        if STATUS.is_man_mode():
            self.add_status(_translate("HandlerClass","Must be in AUTO or MDI mode mode to run a program"), WARNING)
            return

        if STATUS.is_mdi_mode():
            if isinstance(self.CycleFocusWidget, MDI_WIDGET):
                if self.CycleFocusWidget.isVisible():
                    txt = _translate("HandlerClass","Running MDI command")
                    self.add_status("{}: {}".format(txt, str((self.CycleFocusWidget.text()).strip())))
                    self.w.mdiline.submit()
            elif isinstance(self.CycleFocusWidget, self._probeLibrary):
                if self.CycleFocusWidget.isVisible():
                    self.add_status(_translate("HandlerClass","Running Probe routine command"))
                    self.probe.cycle_start()
                else:
                    self.add_status(_translate("HandlerClass","Probe routine cycle start focus error"), CRITICAL, noLog=True)
            else:
                self.add_status(_translate("HandlerClass","No cycle start object selected"), WARNING, noLog=True)
            return

        # in auto mode
        if not  os.path.exists(self.last_loaded_program):
            self.add_status(_translate("HandlerClass","No program to execute"), WARNING, noLog=True)
            return
        if self.w.stackedWidget_mainTab.currentIndex() != 0:
            self.add_status(_translate("HandlerClass","Switch view mode to MAIN"), WARNING, noLog=True)
            return
        if STATUS.is_auto_running():
            self.add_status(_translate("HandlerClass","Program is already running"), WARNING, noLog=True)
            return
